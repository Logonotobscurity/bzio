...
function getStatusColor(status: string): string {
  const colors: Record<string, string> = {
    ...
    isRead: 'bg-gray-100 text-gray-800',
...
function MetricsCards({ stats }: { stats: AdminStats }) {
  return (
  ...
  return (
    <div className="space-y-4">
      {activities.length > 0 ? (
        ...
        <p className="text-center py-8 text-muted-foreground">No recent activities</p>
      )}
    </div>
  );
}
      ...
      <div className="container mx-auto px-4 py-4 md:py-8 space-y-4 md:space-y-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <div className="overflow-x-auto bg-white rounded-lg">
            <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-5 h-auto gap-1 p-1 bg-transparent">
          ...
          <TabsContent value="activity" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Real-time overview of all platform activities and events</CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <CardHeader>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div>
                    ...
                    <CardDescription>Monitor user signups and onboarding status</CardDescription>
                  </div>
                          ...
                          <TableCell>
                            <Badge
                              className={`${getStatusColor(
                                user.emailVerified ? 'verified' : 'pending_verification'
                              )} text-xs`}
                            >
                    ...
                    {newsletterSubscribers.length > 0 ? (
                      newsletterSubscribers.map((sub) => (
                        <TableRow key={sub.id}>
                          ...
                          <TableCell className="text-xs whitespace-nowrap">
                            {sub.unsubscribedAt
                              ? format(new Date(sub.unsubscribedAt), 'MMM dd')
                              : '-'}
                          </TableCell>
                        </TableRow>
                      ))
          ...
          <TabsContent value="forms" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Track all form submissions from customers</CardDescription>
              </CardHeader>
              <CardContent>
                <TableWrapper>
                  <TableHeader>
